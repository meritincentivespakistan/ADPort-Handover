...
const payment = {
    payment(vuexContext) {
        ...
        return axios.get(process.env.baseUrl + 'fetch/payments', header)
    },
    getPlansFaqQuestions(vuexContext) {//محتوى سؤال وجواب صفحة الدفع
        //header.headers.Authorization = vuexContext.state.token
        return axios.get(process.env.baseUrl + 'fetch/contents/faq_payments', header)
    },
    paymentApi(vuexContext) {
        header.headers.Authorization = vuexContext.state.token
        return axios.post(process.env.baseUrl + 'Payment/canPay', '', header)
    },

    setCopouns(vuexContext, number) {
        console.log('====================================');
        console.log(number);
        console.log('====================================');
        return axios.get(process.env.baseUrl + 'fetch/copouns/' + number, header)
    }
}

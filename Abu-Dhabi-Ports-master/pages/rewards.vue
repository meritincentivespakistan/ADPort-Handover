  ...
  <v-app class="BG">
        ...
        <v-layout row justify-space-between>
          ...
          <v-flex xs8>
            <v-layout row justify-space-around>
              <v-flex xs5 class="goal-item">
                <v-card class="fixed-height">
                  <v-card-title primary-title>
                    <div>
                      <div class="headline">
                        Your
                        <span class="bold">Goal Item</span>
                      </div>
                      <div class="description">Set award from the catalogue</div>
                      <div class="points-container">
                        <v-chip color="#0f2a3b" text-color="white" @click="showSearchModal">
                          Set Goal Item
                          <v-progress-circular v-if="SearchLoad" indeterminate color="green"></v-progress-circular>
                        </v-chip>
                      </div>
                    </div>
                  </v-card-title>
              ...
              <v-flex xs7 class="carousel-container" v-if="goalItemProduct">
                <!-- BB -->
                <v-layout row wrap>
                  <v-flex xs7 >
                    <v-card class="margin-2 " >
                      <v-img
                        class="card-image"
                        :src="goalItemProduct.image"
                        style="width: 100%;height: 83%;"
                      ></v-img>
                     
                      <div>
                        <p class="headline  goalItem">{{goalItemProduct.name}}</p>
                        <p class="headline  goalItem">{{goalItemProduct.value}} Points</p>
                      </div>
                    </v-card>
                  </v-flex>
                </v-layout>
    ...
    <template>
      <v-container grid-list-xl>
        <v-layout row justify-space-between style="margin-top:2px">
          <v-flex xs4>
            <ul class="list-group">
              <div v-for="(category,i) in categoriesFirst" :key="i">
                <li class="list-group-item" v-if="category.Sub&&category.Sub.length>0">
                  {{category.name}}
                  <ul class="list-group" v-for="(subCategory,j) in category.Sub" :key="j">
                    <li
                      :id="'cat'+subCategory.id"
                      :class="[{'list-group-item cursor':true},{'active':i==0 && j==0}]"
                      @click="showCategory(subCategory.id)"
                    >{{subCategory.name}}</li>
                  </ul>
                </li>
                <li
                  :id="'cat'+category.id"
                  :class="[{'list-group-item cursor':true},{'active':i==0}]"
                  v-else
                  @click="showCategory(category.id)"
                >{{category.name}}</li>
              </div>
              <!--  -->
              <div v-for="(category,i) in categoriesSecond" :key="i">
                <li class="list-group-item" v-if="category.Sub&&category.Sub.length>0">
                  {{category.name}}
                  <ul class="list-group" v-for="(subCategory,j) in category.Sub" :key="j">
                    <li
                      :id="'cat'+subCategory.id"
                      :class="[{'list-group-item cursor':true}]"
                      @click="showCategory(subCategory.id)"
                    >{{subCategory.name}}</li>
                  </ul>
                </li>
            ...
            <v-flex xs12>
              <v-carousel hide-controls height="250">
          ...
          <v-flex xs8>
            <v-layout row wrap>
              <v-flex xs12 sm6 md4 v-for="(product,i) in products" :key="i">
                <v-card>
                  <v-img class="card-image" :src="product.image" style="width: 100%;height: 83%;"></v-img>
                  <div class="text-xs-center like-chip" v-if="false">
                    <v-chip>
                      <v-avatar>
                        <img src="~/assets/images/notification.svg" alt="trevor">
                      </v-avatar>2
                    </v-chip>
                  </div>
                  <div>
                    <p class="headline card-title height-4">{{ProductName(product.name)}}</p>
                    <div class="card-description">{{product.value}} Points</div>
                  </div>

                  <v-card-actions class="card-footer">
                    <v-btn
                      :disabled="!itemAvailable(product.value)"
                      flat
                ...
                <v-card>
                  <div>
                    <div
                      ...
                      style="padding:20px"
                    >this category doesn't have products yet</div>
                  </div>
                </v-card>
              </v-flex>
            </div>
    ...
    <v-dialog v-model="showModal" max-width="600">
      <v-card>
        ...
        <v-card-text class="center-align">
          <h6>you are about to pay {{points}} points and get this item</h6>
          <p>Please fill this form to continue</p>
          <v-form ref="placeOrder">
            <v-layout wrap>
              <v-flex xs6>
                <v-text-field
                  :rules="required"
              ...
              <v-flex xs6>
                <v-text-field :rules="required" light solo placeholder="email" v-model="email"></v-text-field>
              </v-flex>
              <v-flex xs6>
                <v-text-field :rules="required" light solo placeholder="Phone" v-model="phone"></v-text-field>
              </v-flex>
              <v-flex xs6>
                <v-text-field :rules="required" light solo placeholder="Adress" v-model="address"></v-text-field>
              </v-flex>
              <v-flex xs6>
                <v-text-field :rules="required" light solo placeholder="City" v-model="city"></v-text-field>
              </v-flex>
              <v-flex xs6>
                <v-text-field :rules="required" light solo placeholder="Country" v-model="country"></v-text-field>
              </v-flex>
              <v-flex xs6>
                <v-checkbox v-model="isGift" label="this is a gift" value="1"></v-checkbox>
              </v-flex>
              <v-flex xs12>
        ...
        <v-card-actions>
          <v-btn block flat @click="showModal=false">Cancel</v-btn>
          <v-divider vertical></v-divider>

          <v-btn block flat :disabled="disablePlace" @click="placeOrder">
            Place the Order
            <v-progress-circular v-if="placeOrderLoader" indeterminate color="green"></v-progress-circular>
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="searchModal" max-width="600">
      <v-card class="modalCard">
        <v-card-title class="headline">Choose a reward</v-card-title>
        <v-card-text>
          <v-text-field
            light
            solo
            ...
            @keyup="searchProductFn"
          ></v-text-field>

          <v-layout row wrap class="scroll-y margin-rt-lf-2" style="max-height: 400px">
            <v-flex col xs12 md4 v-for="(product,i) in filteredProducts" :key="i">
              <div :id="'user'+product.id" class="user" @click="sendUserID(product.id)">
                <v-card class="height-auto">
                  <div class="avatar-container">
                    <v-avatar class="card-avatar">
                      <img :src="product.image" alt="John" style="width: 100%;height: 83%;">
                      <span class="g-point"></span>
                    </v-avatar>
                  </div>
                  ...
                  <v-card-action>
                    <v-btn block flat color="green" @click="setGoal(product)">Choose</v-btn>
                  </v-card-action>
                </v-card>
              </div>
            </v-flex>
            <v-flex col xs6 v-if="Allproducts.length==0">
              <v-card class="height-auto">
...
export default {
  ...
  data() {
    return {
      items: [
        {
          src:
            "http://mylist.gyftbox.in/giftcard_image/merchandise/M1211815.jpg"
        },
        {
          src:
            "http://mylist.gyftbox.in/giftcard_image/merchandise/SMGLA9D128BPK.jpg"
        },
        {
          src:
            "http://mylist.gyftbox.in/giftcard_image/merchandise/MMI534AC31IBO.jpg"
        },
        {
          src:
            "http://mylist.gyftbox.in/giftcard_image/merchandise/MKD-5KSM175PSBBY.jpg"
        }
      ],
      categories: null,
      products: null,
      ...
      Allproducts: {},
      SearchLoad: false,
      shouldICallAllProducts: true,
      searchKeyword: "",
      filteredProducts: {},
      goalItemProduct: null,
      UserPoints: 0,
  ...
  mounted() {
    ...
    this.islogged();
    this.$store.dispatch("getCategories").then(resp => {
      console.log("resp", resp);
      this.categories = resp.data.data;
      this.categoriesFirst = resp.data.data.filter(item => {
        return item.id == 19 || item.id == 26;
      });
      this.categoriesSecond = resp.data.data.filter(item => {
        return item.id != 19 && item.id != 26;
      });

      console.log(this.categoriesFirst)
      console.log(this.categoriesSecond)

      let flag = true;
      this.categoriesFirst.forEach((element, i) => {
        if (flag)
          // if (element.Sub == 0) {
            this.initId = element.id;
            console.log("base init" + this.initId);
            flag = false;
          // } else {
          //   this.initId = element.Sub[0].id;
          //   console.log("sub init" + this.initId);
          //   flag = false;
          // }
      });
      this.$store
        .dispatch("getProducts", this.initId)
        .then(resp => {
          console.log("resp", resp);
    ...
    this.$store
      .dispatch("getItemGoal", Cookies.get("userId"))
      .then(resp => {
        this.goalItemProduct = resp.data.data.product;
      })
      .catch(err => {
        console.log("err", err);
      });
      ...
      if (this.$refs.placeOrder.validate()) {
        ...
        this.$store
          ...
          .catch(err => {
            ...
            this.placeOrderDesc = "Please try again";
          });
      }
    },
    logoutFn() {
